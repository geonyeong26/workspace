 ...
 * 정수, 실수, 불리언, 문자열, ... 서비스로 직개발시 다양한 자료형 필요
 ...
 *  
 * 구조체 변수의 크기
 *  - 모든 시스템은 데이터를 빠르게 읽고 쓰기위해 일정한 크기 단위(블록)로 메모리 접근
 *  - 컴파일러는 구조체 멤버의 크기가 들쑥날쑥한 경우 멤버 사이에 패딩 바이트(padding byte)를 사용해서 정렬 → 바이트 얼라인먼트(byte alignment)
